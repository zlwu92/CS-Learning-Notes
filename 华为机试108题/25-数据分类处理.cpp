...
#include <algorithm>
using namespace std;
bool isContain(int n, int k)
{
    string str1 = to_string(n);
    string str2 = to_string(k);
    return (str1.find(str2) == string::npos) ? false : true;
}
int main()
{
    int nI, nR;
        ...
        for(int i = 0; i < R.size(); ++i)
        {
            ...
            for(int j = 0; j < nI; ++j)
            {
                if(isContain(I[j], R[i]))
                {
                    flag = true;
                    ++count;
                    res.push_back(j); // 满足条件的数字对应的index
                    res.push_back(I[j]); // 满足条件的数字
                }
            }
            if(flag) // count > 0
            {
                head.push_back(R[i]);
                head.push_back(res.size()/2);
                head.insert(head.end(), res.begin(), res.end()); // append
            }
        }
        cout << head.size() << " ";
